...
ModUtils.parseIntoArgs = function (argsString) {
    var cleanedString = _s.clean(argsString);
    var tempArgs = cleanedString.match(/(?:[^\s"]+|"[^"]*")+/g);
    var cleanArgs = _.map(tempArgs,function(element){return _s.unquote(element)});
    cleanArgs.shift();
...
ModUtils.getAllPlayers = function () {
...
on("chat:message", function (msg) {
    ...
    var args =  ModUtils.parseIntoArgs(msg.content);
    var token = null;
    var char_id = "";
    var attr_name = "";
    var curVal = "";
    if (args === "!attributeDown") {
        ...
        ModUtils.getAllPlayers();
    } else if (args === "!attributeUp") {
        token = getObj("graphic", args[0]);
        char_id = token.get("represents");
        if (char_id != "") {
            attr_name = args[1];
    ...
    } else if (args === "!setAttribute") {
        var players = ModUtils.getAllPlayers();
        log(args);
        log(cleanedArgs);
        //token = getObj("graphic", args[0]);
        //char_id = token.get("represents");
    ...
    } else if (args === "!setTint") {
        token = getObj("graphic", args[0]);
        var color = args[1];

        token.set("tint_color", color);
    } else if (args === "!addTracker") {
        ...
        log(turnorder);
    } else if (args === "!decrementAttributeForSelected") {
        attr_name = args[0];
    ...
    } else if (args === "!increaseAttributeForSelected") {
        attr_name = args[0];
